...
describe('fromPoint', () => {
    ...
    it('should get the function in the 90º degree', () => {
        expect(fromPoint({ point: { x: -1, y: 1 }, angle: 90 })).toEqual({ a: Number.POSITIVE_INFINITY, b: -1 });
        expect(fromPoint({ point: { x: 0, y: 0 }, angle: 90 })).toEqual({ a: Number.POSITIVE_INFINITY, b: 0 });
        expect(fromPoint({ point: { x: 1, y: -1 }, angle: 90 })).toEqual({ a: Number.POSITIVE_INFINITY, b: 1 });
    });
